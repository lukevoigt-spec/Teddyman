...
(function(){ try{
  ...
  // MAP-1 (extension): the decorative friend ally figures (class="mfriend") sit near the nodes —
  // they too must be pointer-events:none so they never intercept a node tap. .mnode/.portalnode stay tappable.
  // Detected by the stable class="mfriend" hook so it works across both the legacy + ARENA-V2 map paints.
  ...
  (function(){ var mp=mapPaintSVG();
    ...
    var friends=cnt(mp,'class="mfriend"');                             // every decorative friend figure
    var guarded=cnt(mp,'class="mfriend" pointer-events="none"');       // ...that is guarded against taps
...
(function(){
  ...
  ok("Act-2 rescues span >=4 distinct zones (Brody/Daisy/Cal/Bryce/Nora + Kendall)", new Set(zones).size>=4, zones);
  ...
  ok("each Act-2 friend has a signature colour + renderable face (map + win)",
