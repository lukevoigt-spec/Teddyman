...
#### Morpheus production-readiness sweep — 2026-06-16
**Verdict:** latest `main` is broadly playable, but I would not call it production-ready until the no-TTS audio fail-soft bug is fixed. I pulled to `0b0d9b4`, ran `save`, `curriculum`, and `ui-emoji` tests clean, ran a full JS syntax sweep clean, rendered the main WebKit scene set, smoke-started all 122 missions with no page errors, and completed representative find/read/forge/magic-e/syllable/sentence/cloze/scramble paths in WebKit with no page errors.

1. **HIGH - no-TTS WebKit cannot leave title / Voice Studio because `Aud.pick()` is unguarded.** In headless WebKit, `speechSynthesis` is absent (`"speechSynthesis" in window === false`). Dispatching `#btnPlay.click()` leaves the app on `scrTitle` and throws `ReferenceError: Can't find variable: speechSynthesis` from `audio.js:57`, called by `game.js:595`; opening Voice Studio throws the same stack through `game.js:2330`. Source confirms `Aud.pick()` uses bare `speechSynthesis.getVoices()` (`audio.js:57`) while `_tts()` has the intended no-TTS guard (`audio.js:102`). Direct first-touch callers include title PLAY (`game.js:595`), hub Audio (`game.js:2330`), and Voice Test (`game.js:2364`). Proposed fix for Neo: make `Aud.pick()` a no-op when `window.speechSynthesis` or `getVoices` is unavailable, use `window.speechSynthesis` consistently, and keep the iOS first-gesture call path intact. This is a hard-constraint #8 fail-soft issue, even if the target iPad normally has TTS. — Morpheus, 2026-06-16

2. **SHIP-GATE reinforcement for existing #27/#31 - no-emoji compliance is not end-to-end yet.** The guard says it enforces child-facing no-emoji (`tests/ui-emoji.test.js:5`) but is explicitly a ratchet, not a zero-emoji gate (`tests/ui-emoji.test.js:10`) over declared clean zones only (`tests/ui-emoji.test.js:37`). Current child-facing surfaces still render emoji: Vault title (`index.html:180`), Spell Scroll title (`index.html:203`), Sound Warm-Up title (`index.html:211`), win/rest buttons (`index.html:359`, `index.html:370`), combo pop text (`game.js:479`), and Vault full/charged text (`game.js:2116`). This is probably already the spirit of #27/#31, so I would not open a duplicate; I would make it a production gate: either expand the clean-zone test to all child screens with a parent-only allowlist, or replace these with `gicon`/`uiIcon`/painted assets before ship. — Morpheus, 2026-06-16

3. **PERF / boot-risk opportunity - the shipped voice pack is on the critical path.** `voicepack.js` is 28,826,331 bytes and is loaded as a blocking script before the core game modules (`index.html:606`), even though the engine is designed to fall back to TTS. Music also creates an audio element with `preload="auto"` (`music.js:20`) and probes `art/bgm-a*.mp3` at boot (`music.js:24-33`); the current Act-1/Act-2 MP3s are 5,777,595 bytes each. Proposed fix: lazy-load `voicepack.js` after first paint / first interaction, or split it by line category and fetch on demand with TTS fallback; consider `preload="metadata"` or gesture-triggered BGM load. This is not a correctness blocker, but it is a production iPad risk on cold load and every fresh deployment. — Morpheus, 2026-06-16
