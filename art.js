...
const UICONS={
  ...
  /* World Map — premium folded treasure-map (parchment + route + X), glossy game style */
  map:(s=20)=>{const u="ui"+(__huid++);return `<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true"><defs><linearGradient id="${u}p" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fff3cf"/><stop offset="1" stop-color="#e7c581"/></linearGradient></defs><ellipse cx="50" cy="90" rx="34" ry="5" fill="#000" opacity=".25"/><path d="M16 26 L40 20 L64 26 L86 20 L86 74 L64 80 L40 74 L16 80 Z" fill="url(#${u}p)" stroke="${PI_INK}" stroke-width="5.5" stroke-linejoin="round"/><path d="M40 20 V74 M64 26 V80" stroke="${PI_INK}" stroke-width="3" opacity=".45" fill="none"/><path d="M27 62 Q40 40 54 50 Q66 58 73 37" stroke="#c0532a" stroke-width="4" stroke-linecap="round" fill="none" stroke-dasharray="1 9"/><path d="M67 30 l9 9 M76 30 l-9 9" stroke="#c0532a" stroke-width="5" stroke-linecap="round"/><path d="M18 28 L40 22 L40 33 L18 39 Z" fill="#fff" opacity=".32"/></svg>`},
  ...
  /* CHEVRON — forward "continue/next" arrow (replaces ➜); inherits the button's text colour */
  chevron:(s=22)=>`<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true" style="vertical-align:-4px"><path d="M38 20 L70 50 L38 80" fill="none" stroke="currentColor" stroke-width="13" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  /* PLAY — solid forward triangle (replaces ▶ on the title/rest buttons); inherits text colour */
  play:(s=22)=>`<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true" style="vertical-align:-4px"><path d="M30 20 L80 50 L30 80 Z" fill="currentColor" stroke="currentColor" stroke-width="10" stroke-linejoin="round"/></svg>`,
  /* SCROLL — parchment scroll (replaces 📜 on the Spell Scroll title); rolled top+bottom + ink lines */
  scroll:(s=22)=>{const u="ui"+(__huid++);return `<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true" style="vertical-align:-6px"><defs><linearGradient id="${u}p" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fff3cf"/><stop offset="1" stop-color="#e7c581"/></linearGradient></defs><ellipse cx="50" cy="93" rx="27" ry="5" fill="#000" opacity=".22"/><rect x="26" y="18" width="48" height="62" rx="5" fill="url(#${u}p)" stroke="${PI_INK}" stroke-width="5"/><g stroke="#b07b3e" stroke-width="4" stroke-linecap="round"><line x1="36" y1="36" x2="64" y2="36"/><line x1="36" y1="48" x2="64" y2="48"/><line x1="36" y1="60" x2="56" y2="60"/></g><rect x="18" y="12" width="64" height="13" rx="6.5" fill="#d99a2b" stroke="${PI_INK}" stroke-width="4.5"/><rect x="18" y="75" width="64" height="13" rx="6.5" fill="#d99a2b" stroke="${PI_INK}" stroke-width="4.5"/></svg>`},
  /* FLAME — bold tongue of fire (replaces 🔥 on the Sound Warm-Up title + combo chips); warm gradient */
  flame:(s=22)=>{const u="ui"+(__huid++);return `<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true" style="vertical-align:-4px"><defs><linearGradient id="${u}f" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ffe066"/><stop offset=".5" stop-color="#ff7a1a"/><stop offset="1" stop-color="#e0451f"/></linearGradient></defs><path d="M54 6 C58 28 80 36 72 60 a24 24 0 1 1 -44 2 C26 48 36 44 40 52 C40 38 46 22 54 6 Z" fill="url(#${u}f)" stroke="${PI_INK}" stroke-width="5" stroke-linejoin="round"/><path d="M52 56 C58 62 57 72 50 84 C45 74 45 66 52 56 Z" fill="#ffe98a"/></svg>`},
  /* MOON — cozy crescent (replaces 😴 on the Rest button); warm gold */
  moon:(s=22)=>{const u="ui"+(__huid++);return `<svg viewBox="0 0 100 100" width="${s}" height="${s}" aria-hidden="true" style="vertical-align:-4px"><defs><linearGradient id="${u}m" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#fff3b0"/><stop offset="1" stop-color="#ffcf3a"/></linearGradient></defs><path d="M64 12 A40 40 0 1 0 88 64 A30 30 0 1 1 64 12 Z" fill="url(#${u}m)" stroke="${PI_INK}" stroke-width="5" stroke-linejoin="round"/></svg>`},
...
/* ============================================================
   ...
   emoji in BASE_ITEMS with crafted SVG in the house icon language (same as PICONS /
   the characters): PI_INK outline, bold flat fills from the gem palette, a floor
   shadow + a white highlight so each reads as a sculpted object, not a sticker.
   RESOLVER: itemArt(it,size) returns ITEMART[it.id](size) (emoji fallback only if an
   id is ever missing art). Rendered in the shop grid, the trophy shelf, and the
   buy/chest unlock reveal — one art source, every site. ids/costs UNCHANGED (saves
   are keyed by BASE_ITEMS.id; never renumber, #7). */
const SHADOW=`<ellipse cx="50" cy="92" rx="27" ry="5" fill="#000" opacity=".22"/>`;
const EMBLEM=(x,y,r)=>`<polygon points="${x},${y-r} ${x+r*0.87},${y-r*0.5} ${x+r*0.87},${y+r*0.5} ${x},${y+r} ${x-r*0.87},${y+r*0.5} ${x-r*0.87},${y-r*0.5}" fill="#ffd23a" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><text x="${x}" y="${y+r*0.45}" text-anchor="middle" font-family="Bangers,sans-serif" font-size="${r*1.3}" fill="${PI_INK}">T</text>`;
const ITEMART={
  /* Hero Banner — heraldic pennant on a pole, hero gold-hex "T" emblem */
  banner:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<line x1="26" y1="8" x2="26" y2="93" stroke="#8a6a3a" stroke-width="6" stroke-linecap="round"/><circle cx="26" cy="9" r="6" fill="#ffd23a" stroke="${PI_INK}" stroke-width="3"/><path d="M29 14 H84 L73 38 L84 62 H29 Z" fill="#3b82f0" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M29 14 H84 L73 38 Z" fill="#fff" opacity=".14"/>${EMBLEM(55,38,12)}</svg>`,
  /* Gem Cluster — a glowing crystal geode (Teddy's #1 love; was the off-theme 🪴) */
  plant:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<path d="M24 80 Q22 64 36 62 L64 62 Q78 64 76 80 Z" fill="#5a4a78" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><polygon points="38,64 32,34 47,64" fill="#7fd9ff" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><polygon points="63,64 70,30 53,64" fill="#a06ae8" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><polygon points="50,64 50,18 59,42 50,64 41,42" fill="#3ec97e" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M50 22 L56 40 L50 56" fill="#fff" opacity=".28"/></svg>`,
  /* Hero Poster — framed comic poster, emblem + light rays */
  poster:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<rect x="22" y="12" width="56" height="76" rx="5" fill="#2b2438" stroke="${PI_INK}" stroke-width="4"/><rect x="28" y="18" width="44" height="64" rx="2" fill="#3b82f0"/><g stroke="#7fd9ff" stroke-width="3" opacity=".55" stroke-linecap="round"><line x1="50" y1="48" x2="50" y2="22"/><line x1="50" y1="48" x2="34" y2="30"/><line x1="50" y1="48" x2="66" y2="30"/></g>${EMBLEM(50,48,12)}<g fill="#bfe3ff" opacity=".85"><rect x="36" y="70" width="28" height="4" rx="2"/><rect x="40" y="77" width="20" height="3.5" rx="1.7"/></g></svg>`,
  /* Gold Trophy — classic two-handle cup with a star */
  trophy:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<rect x="33" y="83" width="34" height="8" rx="2" fill="#8a6a2b" stroke="${PI_INK}" stroke-width="4"/><rect x="40" y="75" width="20" height="9" fill="#caa23a" stroke="${PI_INK}" stroke-width="4"/><rect x="46" y="64" width="8" height="13" fill="#caa23a" stroke="${PI_INK}" stroke-width="3"/><path d="M30 22 Q15 24 18 42 Q21 54 33 53" fill="none" stroke="${PI_INK}" stroke-width="4"/><path d="M70 22 Q85 24 82 42 Q79 54 67 53" fill="none" stroke="${PI_INK}" stroke-width="4"/><path d="M30 20 H70 V38 Q70 62 50 66 Q30 62 30 38 Z" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M34 24 H50 V60 Q38 56 34 40 Z" fill="#fff" opacity=".18"/><polygon points="50,32 53,40 61,40 55,45 57,53 50,48 43,53 45,45 39,40 47,40" fill="#fff5c0" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  /* Gold Medal — twin ribbons + star medallion */
  medal:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<path d="M40 14 L48 52 L34 48 Z" fill="#e0457b" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M60 14 L66 48 L52 52 Z" fill="#3b82f0" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><circle cx="50" cy="66" r="21" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4"/><circle cx="50" cy="66" r="14" fill="none" stroke="${PI_INK}" stroke-width="2" opacity=".5"/><path d="M40 56 A21 21 0 0 1 56 49" fill="none" stroke="#fff" stroke-width="4" opacity=".4" stroke-linecap="round"/><polygon points="50,56 53,64 61,64 55,69 57,77 50,72 43,77 45,69 39,64 47,64" fill="#fff5c0" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  /* Star Lamp — a glowing lantern with a star flame */
  lamp:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<circle cx="50" cy="50" r="30" fill="#ffd23a" opacity=".18"/><path d="M40 16 Q50 6 60 16" fill="none" stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"/><path d="M34 26 H66 L60 34 H40 Z" fill="#8a6a3a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="36" y="34" width="28" height="40" rx="5" fill="#ffe7a0" stroke="${PI_INK}" stroke-width="4"/><path d="M40 38 H46 V70 H40 Z" fill="#fff" opacity=".4"/><polygon points="50,44 53,52 61,52 55,57 57,65 50,60 43,65 45,57 39,52 47,52" fill="#ffae00" stroke="${PI_INK}" stroke-width="2"/><rect x="33" y="74" width="34" height="9" rx="2" fill="#8a6a3a" stroke="${PI_INK}" stroke-width="4"/></svg>`,
  /* Vexbot Toy — a chibi villain robot (collectible foe) */
  vexbot:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<line x1="50" y1="22" x2="50" y2="12" stroke="${PI_INK}" stroke-width="3"/><circle cx="50" cy="11" r="3.5" fill="#e6453c" stroke="${PI_INK}" stroke-width="2"/><rect x="36" y="46" width="28" height="30" rx="6" fill="#6b7280" stroke="${PI_INK}" stroke-width="4"/><circle cx="50" cy="60" r="5" fill="#e6453c" stroke="${PI_INK}" stroke-width="2"/><rect x="26" y="50" width="9" height="20" rx="4" fill="#9aa3b0" stroke="${PI_INK}" stroke-width="3"/><rect x="65" y="50" width="9" height="20" rx="4" fill="#9aa3b0" stroke="${PI_INK}" stroke-width="3"/><rect x="36" y="22" width="28" height="24" rx="6" fill="#9aa3b0" stroke="${PI_INK}" stroke-width="4"/><rect x="40" y="30" width="20" height="9" rx="3" fill="#e6453c" stroke="${PI_INK}" stroke-width="2.5"/><rect x="43" y="32" width="5" height="5" rx="1" fill="#fff" opacity=".8"/></svg>`,
  /* Dragon Toy — a cute baby dragon companion (green) */
  dragon:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<path d="M24 66 Q10 60 14 50 Q20 58 30 56 Z" fill="#2fae6a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M58 40 Q78 28 84 40 Q74 44 70 52 Z" fill="#2fae6a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><ellipse cx="48" cy="62" rx="22" ry="17" fill="#3ec97e" stroke="${PI_INK}" stroke-width="4"/><ellipse cx="48" cy="68" rx="13" ry="9" fill="#bff0d4"/><circle cx="64" cy="46" r="14" fill="#3ec97e" stroke="${PI_INK}" stroke-width="4"/><path d="M58 34 L60 26 L64 34 Z" fill="#ffd23a" stroke="${PI_INK}" stroke-width="2.5" stroke-linejoin="round"/><path d="M70 34 L72 26 L76 34 Z" fill="#ffd23a" stroke="${PI_INK}" stroke-width="2.5" stroke-linejoin="round"/><circle cx="67" cy="44" r="3.5" fill="${PI_INK}"/><circle cx="68" cy="43" r="1.2" fill="#fff"/><ellipse cx="74" cy="51" rx="3.5" ry="2.4" fill="#2fae6a" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  /* Crown Stand — gold crown on a velvet cushion */
  crown:(s=64)=>`<svg class="shopart" viewBox="0 0 100 100" width="${s}" aria-hidden="true">${SHADOW}<path d="M26 78 Q26 70 50 70 Q74 70 74 78 L74 84 Q74 90 50 90 Q26 90 26 84 Z" fill="#7a2540" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M30 62 L34 36 L42 52 L50 30 L58 52 L66 36 L70 62 Z" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="30" y="60" width="40" height="9" rx="2" fill="#ffcf33" stroke="${PI_INK}" stroke-width="3.5"/><circle cx="38" cy="64" r="2.6" fill="#e6453c" stroke="${PI_INK}" stroke-width="1.5"/><circle cx="50" cy="64" r="2.6" fill="#3b82f0" stroke="${PI_INK}" stroke-width="1.5"/><circle cx="62" cy="64" r="2.6" fill="#3ec97e" stroke="${PI_INK}" stroke-width="1.5"/><circle cx="34" cy="34" r="3" fill="#fff5c0" stroke="${PI_INK}" stroke-width="1.5"/><circle cx="50" cy="28" r="3" fill="#fff5c0" stroke="${PI_INK}" stroke-width="1.5"/><circle cx="66" cy="34" r="3" fill="#fff5c0" stroke="${PI_INK}" stroke-width="1.5"/></svg>`,
...
Object.assign(PICONS,{
  hen:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M58 30 q6 -10 13 -5 q-1 7 -5 10" fill="#e6453c" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><path d="M30 80 Q18 52 44 44 Q50 34 64 40 Q58 46 60 50 Q86 52 80 78 Q78 86 60 86 L42 86 Q32 86 30 80Z" fill="#c8702a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><circle cx="62" cy="48" r="3.2" fill="${PI_INK}"/><path d="M58 52 l-14 -2 12 7z" fill="#ffae00" stroke="${PI_INK}" stroke-width="2.5" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="3" stroke-linecap="round"><line x1="48" y1="86" x2="48" y2="94"/><line x1="64" y1="86" x2="64" y2="94"/></g></svg>`,
  bug:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="3" stroke-linecap="round"><line x1="40" y1="26" x2="34" y2="14"/><line x1="60" y1="26" x2="66" y2="14"/></g><circle cx="50" cy="30" r="13" fill="${PI_INK}"/><path d="M22 60 a28 26 0 0 1 56 0 a28 22 0 0 1 -56 0Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4"/><line x1="50" y1="36" x2="50" y2="82" stroke="${PI_INK}" stroke-width="4"/><g fill="${PI_INK}"><circle cx="36" cy="54" r="4.5"/><circle cx="64" cy="54" r="4.5"/><circle cx="34" cy="70" r="4"/><circle cx="66" cy="70" r="4"/></g></svg>`,
  bus:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="14" y="28" width="72" height="46" rx="9" fill="#ffcf33" stroke="${PI_INK}" stroke-width="4"/><g fill="#bfe3ff" stroke="${PI_INK}" stroke-width="3"><rect x="22" y="36" width="14" height="14" rx="2"/><rect x="43" y="36" width="14" height="14" rx="2"/><rect x="64" y="36" width="14" height="14" rx="2"/></g><rect x="16" y="60" width="68" height="7" fill="#e0457b"/><g fill="${PI_INK}"><circle cx="32" cy="78" r="9"/><circle cx="68" cy="78" r="9"/></g><circle cx="32" cy="78" r="3.5" fill="#fff"/><circle cx="68" cy="78" r="3.5" fill="#fff"/></svg>`,
  bag:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M30 30 a20 20 0 0 1 40 0" fill="none" stroke="${PI_INK}" stroke-width="5"/><rect x="22" y="30" width="56" height="58" rx="14" fill="#4a8fe0" stroke="${PI_INK}" stroke-width="4"/><rect x="34" y="52" width="32" height="26" rx="7" fill="#bfe3ff" stroke="${PI_INK}" stroke-width="3.5"/><line x1="34" y1="44" x2="66" y2="44" stroke="${PI_INK}" stroke-width="3"/></svg>`,
  cap:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M22 58 a28 24 0 0 1 56 0Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M76 58 q16 0 18 9 q-22 5 -42 0Z" fill="#c43a33" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><circle cx="50" cy="36" r="4" fill="${PI_INK}"/></svg>`,
  pan:()=>`<svg class="picicon" viewBox="0 0 100 100"><line x1="78" y1="62" x2="96" y2="70" stroke="${PI_INK}" stroke-width="8" stroke-linecap="round"/><ellipse cx="46" cy="62" rx="34" ry="14" fill="#3a3550" stroke="${PI_INK}" stroke-width="4"/><ellipse cx="44" cy="60" rx="20" ry="9" fill="#fff7e6"/><circle cx="44" cy="59" r="6.5" fill="#ffc233"/></svg>`,
  nut:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M38 18 a18 18 0 0 1 24 0 q8 8 2 20 q8 14 0 26 a20 20 0 0 1 -30 0 q-8 -12 0 -26 q-6 -12 4 -20Z" fill="#d9a45b" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="2" opacity=".55" fill="none"><path d="M40 30 q10 5 20 0"/><path d="M38 50 q12 6 24 0"/><path d="M40 68 q10 5 20 0"/></g></svg>`,
  fan:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 84 L20 36 A36 36 0 0 1 80 36 Z" fill="#ff7eb0" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="3"><line x1="50" y1="84" x2="34" y2="40"/><line x1="50" y1="84" x2="50" y2="34"/><line x1="50" y1="84" x2="66" y2="40"/></g><circle cx="50" cy="84" r="5" fill="#ffcf33" stroke="${PI_INK}" stroke-width="3"/></svg>`,
  mug:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="4" fill="none" stroke-linecap="round" opacity=".7"><path d="M40 20 q-6 6 0 12"/><path d="M54 18 q-6 6 0 12"/></g><rect x="24" y="40" width="44" height="44" rx="9" fill="#fff" stroke="${PI_INK}" stroke-width="4"/><path d="M68 50 q16 0 16 14 t-16 12" fill="none" stroke="${PI_INK}" stroke-width="5"/><rect x="24" y="40" width="44" height="11" fill="#6f4e2e"/></svg>`,
  pot:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="4" fill="none" stroke-linecap="round" opacity=".7"><path d="M42 18 q-6 6 0 12"/><path d="M58 18 q-6 6 0 12"/></g><rect x="22" y="40" width="56" height="12" rx="4" fill="#9aa3b0" stroke="${PI_INK}" stroke-width="4"/><circle cx="50" cy="38" r="3.5" fill="${PI_INK}"/><path d="M26 52 h48 l-5 30 a6 6 0 0 1 -6 5 H37 a6 6 0 0 1 -6 -5Z" fill="#6b7280" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="4" fill="none"><path d="M26 60 h-8"/><path d="M74 60 h8"/></g></svg>`,
  hug:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="50" cy="46" r="30" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4"/><circle cx="40" cy="42" r="4" fill="${PI_INK}"/><circle cx="60" cy="42" r="4" fill="${PI_INK}"/><path d="M38 54 q12 10 24 0" fill="none" stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"/><g fill="#f2b06a" stroke="${PI_INK}" stroke-width="3.5"><ellipse cx="20" cy="74" rx="12" ry="9" transform="rotate(-20 20 74)"/><ellipse cx="80" cy="74" rx="12" ry="9" transform="rotate(20 80 74)"/></g></svg>`,
  bed:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="12" y="36" width="14" height="42" rx="3" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="4"/><rect x="20" y="56" width="70" height="22" rx="5" fill="#b07b3e" stroke="${PI_INK}" stroke-width="4"/><rect x="22" y="50" width="44" height="14" rx="6" fill="#fff" stroke="${PI_INK}" stroke-width="3.5"/><path d="M64 64 h24 v8 a6 6 0 0 1 -6 6 H64Z" fill="#4a8fe0" stroke="${PI_INK}" stroke-width="3.5"/></svg>`,
  net:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="#9aa3b0" stroke-width="2.5"><path d="M28 26 v52M40 26 v54M52 26 v54M64 26 v54M74 27 v52"/><path d="M18 38 h64M18 50 h64M18 62 h64M18 74 h64"/></g><path d="M18 26 h64 v54 h-64Z" fill="none" stroke="${PI_INK}" stroke-width="5" stroke-linejoin="round"/></svg>`,
  bun:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M18 72 q0 -36 32 -36 t32 36Z" fill="#e0a94e" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="14" y="70" width="72" height="14" rx="7" fill="#c8853a" stroke="${PI_INK}" stroke-width="4"/><path d="M34 52 q16 -10 32 0" fill="none" stroke="#fff" stroke-width="3.5" stroke-linecap="round" opacity=".7"/></svg>`
});
/* ---- U5 rollout batch 2: READWORDS2 digraph words ---- */
Object.assign(PICONS,{
  ship:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="45" y="28" width="9" height="22" fill="#e6453c" stroke="${PI_INK}" stroke-width="3"/><rect x="30" y="40" width="38" height="16" rx="2" fill="#fff" stroke="${PI_INK}" stroke-width="3"/><path d="M16 58 h68 l-11 18 a6 6 0 0 1 -5 3 H32 a6 6 0 0 1 -5 -3Z" fill="#2b6fd6" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M12 84 q8 -6 16 0 t16 0 t16 0 t16 0" fill="none" stroke="#7fd9ff" stroke-width="3.5" stroke-linecap="round"/></svg>`,
  fish:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M68 50 L90 33 L86 50 L90 67Z" fill="#ff8a3d" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><ellipse cx="44" cy="50" rx="32" ry="20" fill="#ffae47" stroke="${PI_INK}" stroke-width="4"/><circle cx="26" cy="44" r="4" fill="${PI_INK}"/><path d="M42 38 q9 12 0 24" fill="none" stroke="${PI_INK}" stroke-width="2.5" opacity=".55"/></svg>`,
  chip:()=>`<svg class="picicon" viewBox="0 0 100 100"><g fill="#ffce3a" stroke="${PI_INK}" stroke-width="3"><rect x="35" y="20" width="8" height="40"/><rect x="46" y="15" width="8" height="45"/><rect x="57" y="20" width="8" height="40"/></g><path d="M30 52 h40 l-5 30 a5 5 0 0 1 -5 4 H40 a5 5 0 0 1 -5 -4Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="34" y="60" width="32" height="9" fill="#fff" opacity=".85"/></svg>`,
  duck:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M28 62 q-14 4 -18 -4 q10 -5 18 -2" fill="#f5c000" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><ellipse cx="48" cy="62" rx="30" ry="20" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4"/><circle cx="70" cy="40" r="15" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4"/><circle cx="73" cy="37" r="3" fill="${PI_INK}"/><path d="M83 39 q14 -2 13 5 q-8 4 -13 1Z" fill="#ff8a3d" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/></svg>`,
  sock:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M36 16 H60 V52 Q60 58 66 60 L80 66 Q88 70 84 78 Q80 86 72 82 L46 70 Q36 65 36 54 Z" fill="#5b8def" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="36" y="16" width="24" height="9" fill="#fff"/><rect x="36" y="27" width="24" height="4" fill="#e6453c"/></svg>`,
  ring:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 32 L39 45 L50 56 L61 45Z" fill="#7fd9ff" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><ellipse cx="50" cy="68" rx="21" ry="19" fill="none" stroke="#ffce3a" stroke-width="9"/><ellipse cx="50" cy="68" rx="25.5" ry="23.5" fill="none" stroke="${PI_INK}" stroke-width="3"/><ellipse cx="50" cy="68" rx="16.5" ry="14.5" fill="none" stroke="${PI_INK}" stroke-width="3"/></svg>`,
  king:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="50" cy="60" r="27" fill="#f2c79a" stroke="${PI_INK}" stroke-width="4"/><circle cx="40" cy="58" r="3.5" fill="${PI_INK}"/><circle cx="60" cy="58" r="3.5" fill="${PI_INK}"/><path d="M41 70 q9 6 18 0" fill="none" stroke="${PI_INK}" stroke-width="3" stroke-linecap="round"/><path d="M26 38 L34 24 L42 34 L50 20 L58 34 L66 24 L74 38 Z" fill="#ffce3a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g fill="#e6453c"><circle cx="34" cy="30" r="2.6"/><circle cx="50" cy="26" r="2.6"/><circle cx="66" cy="30" r="2.6"/></g></svg>`,
  shop:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="22" y="44" width="56" height="40" fill="#cdb89a" stroke="${PI_INK}" stroke-width="4"/><path d="M16 44 h68 l-7 -15 H23Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g fill="#fff" opacity=".9"><rect x="28" y="31" width="9" height="13"/><rect x="46" y="31" width="9" height="13"/><rect x="64" y="31" width="9" height="13"/></g><rect x="42" y="60" width="16" height="24" rx="1" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="3"/></svg>`,
  bath:()=>`<svg class="picicon" viewBox="0 0 100 100"><g fill="#bfe3ff" stroke="${PI_INK}" stroke-width="2.5"><circle cx="40" cy="34" r="7"/><circle cx="55" cy="28" r="5"/><circle cx="63" cy="38" r="6"/></g><path d="M14 50 h72 v13 a17 17 0 0 1 -17 17 H31 a17 17 0 0 1 -17 -17Z" fill="#fff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><line x1="14" y1="57" x2="86" y2="57" stroke="${PI_INK}" stroke-width="2.5" opacity=".45"/><g stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"><line x1="28" y1="80" x2="28" y2="88"/><line x1="72" y1="80" x2="72" y2="88"/></g></svg>`,
  chin:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="50" cy="46" r="26" fill="#f2c79a" stroke="${PI_INK}" stroke-width="4"/><path d="M24 48 q4 34 26 34 t26 -34 q-8 9 -26 9 t-26 -9Z" fill="#6b4a2a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M28 30 q22 -14 44 0" fill="#6b4a2a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><circle cx="40" cy="44" r="3.5" fill="${PI_INK}"/><circle cx="60" cy="44" r="3.5" fill="${PI_INK}"/></svg>`,
  wing:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M84 26 Q40 24 18 70 Q44 58 56 60 Q40 68 34 83 Q60 71 70 64 Q84 52 84 26Z" fill="#eef4ff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="2.5" fill="none" opacity=".5"><path d="M70 36 Q50 40 40 60"/><path d="M77 46 Q58 52 48 69"/></g></svg>`,
  dish:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="3.5" stroke-linecap="round"><line x1="22" y1="36" x2="22" y2="80"/><line x1="17" y1="36" x2="17" y2="50"/><line x1="27" y1="36" x2="27" y2="50"/></g><line x1="80" y1="36" x2="80" y2="80" stroke="${PI_INK}" stroke-width="5" stroke-linecap="round"/><circle cx="50" cy="58" r="24" fill="#fff" stroke="${PI_INK}" stroke-width="4"/><circle cx="50" cy="58" r="15" fill="none" stroke="${PI_INK}" stroke-width="2.5" opacity=".45"/></svg>`
});
/* ---- U5 rollout batch 3: READWORDS2 blend words ---- */
Object.assign(PICONS,{
  frog:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="34" cy="32" r="11" fill="#7ed957" stroke="${PI_INK}" stroke-width="4"/><circle cx="66" cy="32" r="11" fill="#7ed957" stroke="${PI_INK}" stroke-width="4"/><circle cx="34" cy="32" r="4" fill="${PI_INK}"/><circle cx="66" cy="32" r="4" fill="${PI_INK}"/><path d="M18 52 a32 24 0 0 1 64 0 q0 22 -32 22 t-32 -22Z" fill="#7ed957" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M35 64 q15 9 30 0" fill="none" stroke="${PI_INK}" stroke-width="3.5" stroke-linecap="round"/></svg>`,
  drum:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"><line x1="28" y1="18" x2="44" y2="50"/><line x1="72" y1="18" x2="56" y2="50"/></g><circle cx="28" cy="16" r="5" fill="#c8853a" stroke="${PI_INK}" stroke-width="3"/><circle cx="72" cy="16" r="5" fill="#c8853a" stroke="${PI_INK}" stroke-width="3"/><path d="M20 54 v18 a30 9 0 0 0 60 0 V54" fill="#e6453c" stroke="${PI_INK}" stroke-width="4"/><ellipse cx="50" cy="54" rx="30" ry="9" fill="#fff" stroke="${PI_INK}" stroke-width="4"/><g stroke="#e6453c" stroke-width="2.5" opacity=".8"><path d="M24 58 l16 14M44 55 l-14 17M58 55 l16 15M76 58 l-16 16"/></g></svg>`,
  flag:()=>`<svg class="picicon" viewBox="0 0 100 100"><line x1="30" y1="12" x2="30" y2="90" stroke="${PI_INK}" stroke-width="5" stroke-linecap="round"/><path d="M30 16 H80 L67 33 L80 50 H30Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/></svg>`,
  crab:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="${PI_INK}" stroke-width="3.5" stroke-linecap="round"><line x1="32" y1="62" x2="14" y2="68"/><line x1="32" y1="68" x2="16" y2="78"/><line x1="68" y1="62" x2="86" y2="68"/><line x1="68" y1="68" x2="84" y2="78"/></g><ellipse cx="50" cy="62" rx="26" ry="17" fill="#e6453c" stroke="${PI_INK}" stroke-width="4"/><path d="M30 52 q-14 -10 -20 -2 q6 9 18 9Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M70 52 q14 -10 20 -2 q-6 9 -18 9Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><circle cx="42" cy="56" r="4.5" fill="#fff" stroke="${PI_INK}" stroke-width="2"/><circle cx="58" cy="56" r="4.5" fill="#fff" stroke="${PI_INK}" stroke-width="2"/><circle cx="42" cy="56" r="2" fill="${PI_INK}"/><circle cx="58" cy="56" r="2" fill="${PI_INK}"/></svg>`,
  star:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 12 L61 39 L90 41 L67 59 L75 88 L50 71 L25 88 L33 59 L10 41 L39 39Z" fill="#ffce3a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/></svg>`,
  hand:()=>`<svg class="picicon" viewBox="0 0 100 100"><g fill="#f2c79a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"><path d="M34 56 q-13 -2 -15 9 q9 8 17 3Z"/><rect x="33" y="48" width="36" height="34" rx="13"/><rect x="35" y="22" width="7.5" height="30" rx="3.7"/><rect x="44.5" y="15" width="7.5" height="37" rx="3.7"/><rect x="54" y="17" width="7.5" height="35" rx="3.7"/><rect x="63.5" y="23" width="7.5" height="29" rx="3.7"/></g></svg>`,
  jump:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="50" cy="20" r="11" fill="#ffd23a" stroke="${PI_INK}" stroke-width="4"/><path d="M50 32 V56" stroke="${PI_INK}" stroke-width="6" stroke-linecap="round"/><path d="M50 38 L30 24 M50 38 L70 24" stroke="${PI_INK}" stroke-width="6" stroke-linecap="round"/><path d="M50 56 L34 78 M50 56 L66 78" stroke="${PI_INK}" stroke-width="6" stroke-linecap="round"/><path d="M20 90 q15 -6 60 0" fill="none" stroke="#7fd9ff" stroke-width="3" stroke-linecap="round" opacity=".7"/></svg>`,
  lamp:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="#ffce3a" stroke-width="4" stroke-linecap="round"><line x1="50" y1="6" x2="50" y2="15"/><line x1="22" y1="16" x2="29" y2="23"/><line x1="78" y1="16" x2="71" y2="23"/></g><path d="M34 40 a16 16 0 1 1 32 0 q0 12 -8 18 H42 q-8 -6 -8 -18Z" fill="#ffe27a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="42" y="58" width="16" height="14" rx="2" fill="#9aa3b0" stroke="${PI_INK}" stroke-width="3.5"/><line x1="42" y1="64" x2="58" y2="64" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  tent:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 24 L84 80 H16Z" fill="#e6724a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M50 42 L63 80 H37Z" fill="#3a3550" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><line x1="50" y1="16" x2="50" y2="26" stroke="${PI_INK}" stroke-width="3" stroke-linecap="round"/><circle cx="50" cy="15" r="3.2" fill="#ffce3a" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  mask:()=>`<svg class="picicon" viewBox="0 0 100 100"><circle cx="50" cy="48" r="30" fill="#f2c79a" stroke="${PI_INK}" stroke-width="4"/><circle cx="40" cy="42" r="3.5" fill="${PI_INK}"/><circle cx="60" cy="42" r="3.5" fill="${PI_INK}"/><g stroke="${PI_INK}" stroke-width="3" fill="none"><path d="M24 52 L13 46"/><path d="M76 52 L87 46"/></g><path d="M22 50 h56 v8 a28 23 0 0 1 -56 0Z" fill="#fff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="${PI_INK}" stroke-width="2" opacity=".45"><line x1="34" y1="59" x2="66" y2="59"/><line x1="34" y1="65" x2="66" y2="65"/></g></svg>`,
  sled:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M18 54 h62 v8 a6 6 0 0 1 -6 6 H24 a6 6 0 0 1 -6 -6Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M14 80 q-3 -10 8 -10 h56 q11 0 8 10" fill="none" stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"/><g stroke="${PI_INK}" stroke-width="3.5"><line x1="30" y1="68" x2="29" y2="74"/><line x1="70" y1="68" x2="71" y2="74"/></g><path d="M18 54 q-9 -10 -15 -7" fill="none" stroke="#8a5a2b" stroke-width="4" stroke-linecap="round"/></svg>`,
  gift:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="22" y="44" width="56" height="40" rx="3" fill="#e6453c" stroke="${PI_INK}" stroke-width="4"/><rect x="19" y="34" width="62" height="13" rx="3" fill="#ff8aa8" stroke="${PI_INK}" stroke-width="4"/><rect x="44" y="34" width="12" height="50" fill="#ffce3a" stroke="${PI_INK}" stroke-width="3"/><path d="M50 34 q-17 -18 -23 -6 q-4 11 23 6Z" fill="#ffce3a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M50 34 q17 -18 23 -6 q4 11 -23 6Z" fill="#ffce3a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/></svg>`,
  trap:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="20" y="46" width="60" height="34" rx="4" fill="#c8853a" stroke="${PI_INK}" stroke-width="4"/><path d="M32 64 L32 46 L70 52" fill="none" stroke="#9aa3b0" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/><circle cx="32" cy="64" r="5" fill="#6b7280" stroke="${PI_INK}" stroke-width="3"/><circle cx="62" cy="64" r="7" fill="#ffce3a" stroke="${PI_INK}" stroke-width="3"/><circle cx="60" cy="62" r="1.6" fill="${PI_INK}"/><circle cx="65" cy="66" r="1.6" fill="${PI_INK}"/></svg>`,
  milk:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M35 44 h30 l-2 36 a6 6 0 0 1 -6 5 H43 a6 6 0 0 1 -6 -5Z" fill="#fff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M34 22 h32 l-1 22 H35Z" fill="#eef4ff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="34" y="22" width="32" height="7" fill="#bfe3ff"/></svg>`
});
/* ---- U5 rollout batch 4: READWORDS2 magic-e + vowel-team words ---- */
Object.assign(PICONS,{
  cake:()=>`<svg class="picicon" viewBox="0 0 100 100"><g stroke="#e6453c" stroke-width="4" stroke-linecap="round"><line x1="38" y1="14" x2="38" y2="30"/><line x1="50" y1="12" x2="50" y2="30"/><line x1="62" y1="14" x2="62" y2="30"/></g><g fill="#ffce3a"><circle cx="38" cy="12" r="3"/><circle cx="50" cy="10" r="3"/><circle cx="62" cy="12" r="3"/></g><rect x="22" y="46" width="56" height="34" rx="4" fill="#ff8aa8" stroke="${PI_INK}" stroke-width="4"/><path d="M22 48 q14 11 28 0 t28 0 v-8 q-14 11 -28 0 t-28 0Z" fill="#fff7e6" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/></svg>`,
  bike:()=>`<svg class="picicon" viewBox="0 0 100 100"><g fill="none" stroke="${PI_INK}" stroke-width="4"><circle cx="26" cy="66" r="16"/><circle cx="74" cy="66" r="16"/></g><path d="M26 66 L46 66 L60 44 M46 66 L60 44 L74 66 M58 44 H68 M40 44 H52" fill="none" stroke="#e6453c" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  gate:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="30" y="16" width="40" height="68" rx="3" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="4"/><rect x="36" y="24" width="28" height="24" rx="2" fill="none" stroke="${PI_INK}" stroke-width="3" opacity=".55"/><rect x="36" y="54" width="28" height="24" rx="2" fill="none" stroke="${PI_INK}" stroke-width="3" opacity=".55"/><circle cx="62" cy="52" r="3.5" fill="#ffce3a" stroke="${PI_INK}" stroke-width="2"/></svg>`,
  kite:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 12 L74 44 L50 70 L26 44Z" fill="#4a8fe0" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M50 12 L50 70 M26 44 L74 44" stroke="${PI_INK}" stroke-width="2.5" opacity=".55"/><path d="M50 70 q-6 10 2 16 q-8 4 -2 12" fill="none" stroke="${PI_INK}" stroke-width="2.5"/><g fill="#e6453c"><path d="M52 78 l7 3 -7 3z"/><path d="M52 90 l7 3 -7 3z"/></g></svg>`,
  home:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 18 L86 50 H14Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="24" y="50" width="52" height="34" fill="#f2d6a8" stroke="${PI_INK}" stroke-width="4"/><rect x="42" y="62" width="16" height="22" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="3"/><rect x="30" y="58" width="11" height="11" fill="#bfe3ff" stroke="${PI_INK}" stroke-width="2.5"/></svg>`,
  cube:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M30 38 L50 28 L70 38 L50 48Z" fill="#d6f0ff" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M30 38 L30 70 L50 80 L50 48Z" fill="#9bd6f5" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M70 38 L70 70 L50 80 L50 48Z" fill="#7fc4ee" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M36 36 l8 -4" stroke="#fff" stroke-width="3" stroke-linecap="round" opacity=".8"/></svg>`,
  nose:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M50 20 q-6 24 -16 38 q-4 10 6 12 q10 4 20 0 q10 -2 6 -12 q-10 -14 -16 -38Z" fill="#f2c79a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g fill="${PI_INK}"><ellipse cx="42" cy="66" rx="3.6" ry="2.6"/><ellipse cx="58" cy="66" rx="3.6" ry="2.6"/></g></svg>`,
  rose:()=>`<svg class="picicon" viewBox="0 0 100 100"><line x1="50" y1="54" x2="50" y2="88" stroke="#3a8a3a" stroke-width="5" stroke-linecap="round"/><path d="M38 74 q-15 -2 -17 -15 q15 0 19 11Z" fill="#5fb85f" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><path d="M62 78 q15 -2 17 -15 q-15 0 -19 11Z" fill="#5fb85f" stroke="${PI_INK}" stroke-width="3" stroke-linejoin="round"/><circle cx="50" cy="36" r="22" fill="#e6453c" stroke="${PI_INK}" stroke-width="4"/><g fill="none" stroke="${PI_INK}" stroke-width="2.5" opacity=".55"><circle cx="50" cy="36" r="11"/><path d="M50 25 q10 6 0 22 q-10 -6 0 -22"/></g></svg>`,
  rain:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M30 52 a16 16 0 0 1 4 -32 a20 20 0 0 1 37 6 a14 14 0 0 1 -3 26Z" fill="#cfd8e6" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g stroke="#4a8fe0" stroke-width="5" stroke-linecap="round"><line x1="36" y1="62" x2="32" y2="78"/><line x1="52" y1="62" x2="48" y2="80"/><line x1="68" y1="62" x2="64" y2="78"/></g></svg>`,
  train:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="40" y="24" width="14" height="16" rx="1" fill="#3a3550" stroke="${PI_INK}" stroke-width="3"/><path d="M30 40 h46 v26 H16 V52 a12 12 0 0 1 12 -12Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><rect x="34" y="46" width="14" height="13" rx="1" fill="#bfe3ff" stroke="${PI_INK}" stroke-width="2.5"/><g fill="${PI_INK}"><circle cx="30" cy="74" r="8"/><circle cx="60" cy="74" r="8"/></g><circle cx="30" cy="74" r="3" fill="#fff"/><circle cx="60" cy="74" r="3" fill="#fff"/></svg>`,
  feet:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M38 22 q22 -4 24 18 q2 18 -4 30 q-4 10 -16 8 q-12 -2 -12 -16 q0 -8 4 -16 q-2 -20 4 -24Z" fill="#f2c79a" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><g fill="#f2c79a" stroke="${PI_INK}" stroke-width="2.5"><circle cx="64" cy="34" r="5"/><circle cx="68" cy="44" r="4.5"/><circle cx="68" cy="54" r="4"/><circle cx="66" cy="63" r="3.5"/></g></svg>`,
  tree:()=>`<svg class="picicon" viewBox="0 0 100 100"><rect x="45" y="56" width="10" height="30" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="4"/><path d="M50 12 a26 26 0 0 1 22 38 a20 20 0 0 1 -44 0 a26 26 0 0 1 22 -38Z" fill="#4caf50" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/></svg>`,
  bee:()=>`<svg class="picicon" viewBox="0 0 100 100"><g fill="#dff3ff" stroke="${PI_INK}" stroke-width="3"><ellipse cx="38" cy="32" rx="13" ry="9" transform="rotate(-22 38 32)"/><ellipse cx="62" cy="32" rx="13" ry="9" transform="rotate(22 62 32)"/></g><ellipse cx="50" cy="58" rx="23" ry="18" fill="#ffce3a" stroke="${PI_INK}" stroke-width="4"/><g stroke="${PI_INK}" stroke-width="5"><line x1="44" y1="43" x2="44" y2="73"/><line x1="56" y1="43" x2="56" y2="73"/></g><circle cx="40" cy="52" r="2.5" fill="${PI_INK}"/></svg>`,
  boat:()=>`<svg class="picicon" viewBox="0 0 100 100"><line x1="50" y1="14" x2="50" y2="60" stroke="${PI_INK}" stroke-width="4" stroke-linecap="round"/><path d="M50 18 L50 56 L24 56Z" fill="#fff" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M54 24 L54 56 L74 56Z" fill="#e6453c" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M18 62 h64 l-10 16 a6 6 0 0 1 -5 3 H33 a6 6 0 0 1 -5 -3Z" fill="#8a5a2b" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><path d="M12 86 q8 -6 16 0 t16 0 t16 0 t16 0" fill="none" stroke="#7fd9ff" stroke-width="3.5" stroke-linecap="round"/></svg>`,
  goat:()=>`<svg class="picicon" viewBox="0 0 100 100"><path d="M34 26 q-10 -10 -16 -4 q4 12 16 12Z" fill="#cdb89a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M66 26 q10 -10 16 -4 q-4 12 -16 12Z" fill="#cdb89a" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/><path d="M34 38 q16 -10 32 0 q4 18 -2 30 q-6 10 -14 10 t-14 -10 q-6 -12 -2 -30Z" fill="#e8ddc8" stroke="${PI_INK}" stroke-width="4" stroke-linejoin="round"/><circle cx="42" cy="50" r="3.2" fill="${PI_INK}"/><circle cx="58" cy="50" r="3.2" fill="${PI_INK}"/><ellipse cx="50" cy="66" rx="6" ry="4" fill="#c8a98a" stroke="${PI_INK}" stroke-width="2"/><path d="M44 78 q6 12 12 0" fill="#e8ddc8" stroke="${PI_INK}" stroke-width="3.5" stroke-linejoin="round"/></svg>`,
