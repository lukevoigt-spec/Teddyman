...
*(empty — all Morpheus 2026-06-17 sweeps reconciled against current `main`; see the ledger entry "Morpheus 06-17 sweep reconcile" directly below. Cypher's incoming `cypher/qa-deep-dive-2` sweep will be gated here when its PR opens.)*

## ✅ RESOLVED LEDGER

One line each; long detail/spec blocks were removed on consolidation.

- ✅ **Morpheus 06-17 sweep reconcile (Trinity, 2026-06-17):** verified the full day's Morpheus Inbox against current `main` before re-filing — most findings were ALREADY FIXED by the fast-moving crew. (1) sound-ID re-lock accepting early taps → FIXED: `sidArm(pr,row)` now arms rows only after the target-audio promise settles (`nextFind`/`bossRound`/`fortSound`/`vaultFind`); `__sidAt` set at real unlock, `SID_MIN_MS=450` floor on top. (2) fortress correct-handlers double-firing → FIXED: shared one-shot `fortAccept(row,fn)` guards every fortress correct branch (`fortSound`/`fortRead`/`fortSpell`/`fortMaze`/`fortSentencePic`). (3) win-chest invisible (CSS `display:none`) → FIXED: `winChestPop()` sets `box.style.display="block"`. WebKit hero flip-card mirrored → shipped as **#136** (merged). Go-live deep sweep → **#110/#111/#112/#113** (all merged). Refresh-sweep perf notes → tracked under **#107** (voicepack/BGM cold-load). Still genuinely OPEN, filed as Issues: **#162** (map node hit targets <96px in portrait — hard-constraint #6, geometry `map.js` 68×96 rect measured ~57×59 CSS px) + **#163** (Oracle render-verify: Gem-Dex CTA clipped at rest + earned-chest win overflows landscape viewport + the `winchest` harness scene doesn't seed `S.chests` so the gate can't see it).
