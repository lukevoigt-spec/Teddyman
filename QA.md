...
*(new Morpheus note below; prior production sweep → #106/#107; his 2026-06-17 go-live sweep → **converted to Issues #110–#113** by Trinity, detail retained below as the analysis. All four were VERIFIED against code before filing.)*

#### Morpheus deep QA sweep — 2026-06-17
**Verdict:** refreshed `main` through `cb0ad72` (#145 squishy economy v2 landed while this note was being pushed; #152/#153 still present), checked open PRs/issues, ran `save` (132), `curriculum` (125), `ui-emoji` (25), JS/Worker syntax, the broad WebKit render sweep, and a WebKit startup sweep across all 122 missions. The app is broadly playable, but I would not call the latest #153/map work production-clean until the items below are fixed.

1. **HIGH/MED — #153's sound-ID re-lock still accepts guesses before the initial target sound has actually played.** `narrate()` returns the `Aud.play()` promise (`audio.js:140-143`), but `nextFind()` ignores that promise: it starts `["find_prompt","snd_"+g]`, immediately renders choices, and calls `sidStart()` right away (`game.js:1134-1151`). `sidReject()` only blocks taps for `SID_MIN_MS=450` ms after that timestamp (`game.js:1114-1126`), so a fast tap after 450 ms can be accepted even while the prompt audio is still unresolved. A WebKit probe patched `Aud.play()` to a never-resolving promise, ran `startFind("s",5)`, waited about 620 ms, and clicked the correct `s`; the app credited the rep (`findRep=1`, `S.mastery.s.seen=1, ok=1`, `S.xp=1`) while the initial `["find_prompt","snd_s"]` promise had never completed. The same early-arm pattern exists in `bossRound()` (`game.js:1160-1183`), `fortSound()` (`game.js:1399-1411`), and `vaultFind()` (`game.js:2529-2542`). This cuts against the audio-first flow and the anti-gaming purpose: speed still wins if the child taps after the 450 ms floor but before processing the sound. **Proposed fix:** arm answer rows only after the target-audio promise settles (or a short watchdog fires), and keep the row in `.sidwait`/`pointer-events:none` until then. Set `__sidAt` at the actual unlock time, not at render time. Apply the helper consistently to `nextFind`, `bossRound`, `fortSound`, and `vaultFind`, and add a WebKit test with a deliberately unresolved `Aud.play()` that verifies a tap cannot credit mastery until the arm helper releases. — Morpheus, 2026-06-17

2. **MED — finale correct-answer handlers still double-fire, so rapid taps can remove two fortress HP and credit two reps from one prompt.** The ordinary mission handlers mostly lock their row before async narration (`startFind()` correct path at `game.js:1142-1146`, `nextRead()` at `game.js:1215-1218`, sentence/cloze at `game.js:1301-1326`). The fortress finale does not: `fortHit()` decrements HP and increments `fRound` (`game.js:1390-1393`), while the correct paths in `fortSound()` (`game.js:1406-1407`), `fortRead()` (`game.js:1418-1419`), `fortMaze()` (`game.js:1460-1462`), and `fortSentencePic()` (`game.js:1471-1472`) call `record()`/`fortHit()` with no row lock or per-round guard. A WebKit probe held the post-hit audio promise open, rendered a `fortSound()` round, and clicked the correct `s` twice; result: `fRound=2`, `fHP=4`, `S.mastery.s.seen=2, ok=2`, `S.xp=2`, and the correct button still had `pointer-events:auto`. **Proposed fix:** add a shared one-shot guard for fortress rounds, e.g. `fortAccept(row, fn)` that locks `#fortChoices` and refuses any second correct path until `fortRound()` rebuilds. Use it for every fortress correct branch, including the sentence/maze async `flow()` path; for `fortSpell()`, lock/guard the row when the final slot completes so the completion cannot be re-entered. — Morpheus, 2026-06-17

3. **MED — map node hit targets shrink below the project's ~96px touch-target constraint, especially in portrait.** The hard constraint is explicit (`CLAUDE.md:128`), but the current painted-map markers are a 68×96 transparent SVG rect inside `mapBanner()` (`map.js:153-164`) and the entire SVG is scaled to fit the viewport (`styles.css:256-257`). In the latest portrait map fix, `mapPaintV2()` switches to `preserveAspectRatio="meet"` when `innerHeight > innerWidth` (`map.js:197-205`), which keeps nodes visible but scales the hit boxes down. WebKit measurements: on 768×1024 portrait, the current node measured about 57×59 CSS px and locked nodes about 33×47; on 1024×768 landscape, the current node measured about 86×89 and locked nodes about 50×71. The current node is the child's main "play next" action on the map, so this is a real iPad ergonomics risk even though all nodes are onscreen. **Proposed fix:** keep the visual parchment sign size, but increase the interactive hit geometry in SVG units enough to produce at least ~96 CSS px in portrait (roughly a 190px+ map-space hit rect at the current scale), or overlay absolutely positioned HTML buttons with CSS `min-width/min-height:96px` and transparent visuals. Add a portrait geometry assertion to the render gate so "visible" does not get mistaken for "large enough to tap." — Morpheus, 2026-06-17

4. **LOW — the Gem-Dex modal is usable but starts with its primary Recharge action clipped below the iPad landscape viewport.** `openCollectionCard("gems")` renders the full 26-gem grid plus `#collAction` at the bottom (`game.js:2002-2029`), inside a scrollable `.coll-card` with `max-height:88vh` (`styles.css:1529-1531`; markup at `index.html:254-260`). In WebKit 1024×768 with 13 learned gems, the card measured `top=46,bottom=722`, but `#collAction` started at `top=723,bottom=817`, so the CTA is not fully visible at rest. Scrolling the card exposes the CTA (`top=600,bottom=694`), but the close button scrolls out of view (`top=-87,bottom=-41`). Not a dead-end, but it misses the premium/reward-screen polish bar for the new #126/#152 collection loop. **Proposed fix:** make the action a sticky footer inside the modal, keep the close button fixed to the card frame, or reduce the grid/padding enough that the 26-slot Gem-Dex plus CTA fit cleanly at 1024×768. Re-render `gemdex` after the fix. — Morpheus, 2026-06-17

#### Morpheus WebKit reward-card regression — 2026-06-17
1. **MED — the Teddy/hero flip card opens mirrored/backwards in WebKit, so iPad users see unreadable back-face text instead of the hero card front.** `openTeddyCard()` explicitly opens the card with `.flip` removed (`game.js:1766-1774`), and the shared markup puts `.hc-front` before `.hc-back` (`index.html:572-589`). The CSS relies on 3D backface hiding (`.hc-flip{transform-style:preserve-3d}`, `.hc-face{backface-visibility:hidden;-webkit-backface-visibility:hidden}`, `.hc-back{transform:rotateY(180deg)}` at `styles.css:949-957`). On the current WebKit harness, `tools/shots/webkit-teddycard.png` and `tools/shots/webkit-a2herocard.png` render the default open card as the mirrored back face; the same scenes render correctly in Chromium (`tools/shots/chromium-teddycard.png`, `tools/shots/chromium-a2herocard.png`). Because the production iPad target is Safari/WebKit, this is a real go-live visual regression, not just a harness cosmetic. **Proposed fix:** make the card flip WebKit-safe before shipping: first try the missing prefixed 3D preservation (`-webkit-transform-style:preserve-3d` on `.hc-flip`), but if Safari still exposes the rotated back face, add explicit state-based face hiding such as `.hc-flip:not(.flip) .hc-back{visibility:hidden}` and `.hc-flip.flip .hc-front{visibility:hidden}` while preserving the tap target. Re-render `teddycard`, `teddycardback`, and `a2herocard` in WebKit after the fix. — Morpheus, 2026-06-17
...
2. **MED — approved Training-Interrupt line edits did not reach `data-lines.js` (streak/fixed-mindset drift).** `TRAINING-INTERRUPTS.md` says the no-streak/process-praise edits are approved and need syncing: `train_arch17`, `train_arch20`, `train_nora5`, `train_nora18` (`TRAINING-INTERRUPTS.md:16-18`). The shipped line bank still says “Keep that streak alive” (`data-lines.js:326`), “SMART” (`data-lines.js:329`), “getting smarter” (`data-lines.js:374`), and “I always knew you were smart” (`data-lines.js:387`). This cuts against the mastery/process-praise contract and the no-streak-loss spirit in `CLAUDE.md`. **Proposed fix:** replace those four entries with the approved wording from `TRAINING-INTERRUPTS.md`, then add a small curriculum/test assertion that the training line bank has no `streak`, `smart`, or `smarter` outside allowed joke contexts. — Morpheus, 2026-06-17

3. **MED — the in-app Playtest Notes capture does not write to the documented durable log.** `AGENTS.md` says playtest notes land in `PLAYTEST.md` (`AGENTS.md:31-40`), and `PLAYTEST.md` says the in-app box commits a timestamped entry to “this file” and prepends new entries there (`PLAYTEST.md:7-27`). The shipped implementation instead creates one new file per note under `playtest/<ISO>.md` (`playtest.js:1-6`, `playtest.js:42-53`); there is no `playtest/` directory in the repo today. Because real playtest notes outrank all agent QA, this mismatch can make notes invisible to anyone checking the official log, especially if the token lacks Issues write and only the file commit succeeds. **Proposed fix:** either update `playtest.js` to append/prepend `PLAYTEST.md` with a safe SHA-retry contents update, or explicitly change `AGENTS.md`/`PLAYTEST.md` to make `playtest/*.md` plus trigger Issues the canonical intake and add that folder/index to Trinity’s re-engagement checklist. — Morpheus, 2026-06-17

4. **LOW/MED perf — Hero Base entry pulls a large art burst and still does a stale JPEG probe despite a comment saying that was fixed.** A WebKit local cold trace showed title boot requesting ~36.4 MB (already covered by #107: `voicepack.js` + BGM). First Hero Base entry then requested ~8.5 MB more, led by `bg-base-room.png` (2.75 MB), `fists.png` (1.44 MB), and all three cape PNGs (1.33-1.55 MB each). Source: `paintBase()` creates the fists/weapon image and all cape images at once (`game.js:1622-1643`). It also 404-probes `art/bg-base-room.jpeg` before loading the real PNG because `BG_EXTS=["jpeg","png","jpg","webp"]` (`game.js:399-423`), contradicting the “base-room is .png first” comment. **Proposed fix:** after #107, add an asset budget pass: special-case known PNG scene backgrounds or reorder probes per slot, compress the large loadout PNGs (WebP/quantized PNG), and lazy-load locked/hidden loadout art so the Base paints before optional cosmetics finish. — Morpheus, 2026-06-17

#### Morpheus refresh sweep — 2026-06-17
**Verdict:** I refreshed `origin/main` through `51229e2` (#112/#113 landed while this note was being pushed), checked the open PR queue, and reran the hard-constraint smoke pass after the latest fixes. `save` (121), `curriculum` (115), `ui-emoji` (17), full JS syntax, Worker syntax, WebKit render/start sweeps, no-`speechSynthesis` title play, painted-content coverage (107/107 glyphs), and a post-#113 cold Base trace were clean for correctness. No new save-migration, anti-gaming, no-hang, or progression blocker surfaced beyond the already-open/PR-backed items.

1. **LOW/MED perf — #107 removed parser blocking, but `voicepack.js` is still fetched immediately on title boot.** `index.html` now uses `<script defer src="voicepack.js">` (`index.html:605-608`), which is safer than the old blocking script, but `defer` still starts the 28.8 MB request during initial page parse. A post-`51229e2` WebKit cold trace still recorded `/voicepack.js` (28,826,331 bytes) in the title-boot request set. The code can tolerate later loading because `clipFor()` falls back when `VOICEPACK` is absent (`audio.js:45-47`, `audio.js:86`), and the BGM side of #107 did move to `preload="metadata"`/`loadedmetadata` (`music.js:22`, `music.js:32-38`). **Proposed fix:** if real-iPad cold loads still feel heavy, lazy-load `voicepack.js` after first paint or first gesture, or split a tiny critical voice pack from optional line clips; keep TTS fallback for any line requested before the pack arrives. — Morpheus, 2026-06-17

2. **LOW perf follow-up — #113 fixed the stale Base JPEG probe, but the first Base visit still downloads the large loadout art set.** After `51229e2`, the WebKit cold trace has no 404 misses, and the source now tries PNG first for `base-room` (`game.js:404-415`). But first Base entry still requested ~8.5 MB: `bg-base-room.png` (2.75 MB), `fists.png` (1.44 MB), and all three cape PNGs (1.33-1.55 MB). The merged fix correctly marks those loadout images `loading="lazy" decoding="async" fetchpriority="low"` and notes thumbnail byte reduction remains (`game.js:1633-1649`); because the picker is visible on entry, lazy priority does not remove the download. **Proposed fix:** keep #113 closed for the 404/priority fix, but track a follow-up art-budget slice for compressed thumbnail variants/WebP if real-iPad Base entry still stutters. — Morpheus, 2026-06-17

3. **Coordination/readiness note — the remaining major prior risks are mostly fixed in open PRs, not in deployed `main` yet.** `origin/main` now includes #112/#113, but the fixes for #102/#104, #110/#111, and the Hero Base growth pass are still open as PRs #116-#118 and #123. That means the current iPad-deployed branch still carries the known emoji-guard/UI-emoji and roster/training issues until Trinity/Neo land and reconcile that queue. **Proposed fix:** merge/reconcile the open fix PRs, then rerun this same smoke pass on the merged head before calling `main` production-clean. — Morpheus, 2026-06-17

#### Morpheus win-screen chest regression - 2026-06-17
1. **MED - earned treasure chests never appear on the win screen, so #127's instant-reward path is currently invisible on iPad/main.** `showWin()` calls `winChestPop(__earnedChest)` before clearing the earned tier (`game.js:1538`, `game.js:1577`). `winChestPop()` verifies the pending chest, injects a `.winchest` button, and tries to show the container with `box.style.display=""` (`game.js:1926-1934`). But the stylesheet default is `#winChest{ display:none; margin:2px auto; }` (`styles.css:1433`), so clearing the inline display leaves the element hidden by CSS. A WebKit interaction probe on latest `main` (`192789a`) seeded `S.chests.silver=1`, set `__earnedChest="silver"`, and called `showWin(true)`; the DOM contained the button and text (`hasButton:true`, `text:"TAP TO OPEN!"`), but `getComputedStyle(#winChest).display` was still `"none"` and both the container/button had zero rects, so Playwright could not click `.winchest`. **Proposed fix:** make the visible state explicit, either `box.style.display="flex"` in the show path or a `.ready`/`.on` class matched by CSS; then add a small WebKit/UI smoke that seeds an earned chest, asserts `.winchest` is visible/clickable, and verifies `S.chests[tier]` decrements plus the Base fallback still shows unopened chests. - Morpheus, 2026-06-17

#### Morpheus production go-live sweep - 2026-06-17
1. **MED - the #143 win-chest placement fix makes the chest visible, but a real earned-chest win now overflows the iPad landscape viewport and clips both the title and primary Continue button.** The current DOM puts `#winChest` directly after `#winHero` (`index.html:360-371`) inside the shared centered `.screen` column (`styles.css:22`). That makes the reward visible, but the full win stack no longer fits when a chest is actually pending. On `main` at `2003fc2`, a WebKit 1024x768 probe seeded `S.chests.gold=1`, set `__earnedChest="gold"`, and called `showWin(false)`: `#winChest` was visible at `top=295.8,bottom=436.8`, but `VICTORY!` was clipped at `top=-18.5` and `#btnWinMap` ended below the viewport at `bottom=783.4` while the screen stayed non-scrollable (`scrollTop` remained 0). This is not a hard dead-end because most of the button remains tappable, but it violates the large-clear-target/premium reward-screen bar right where the game pays off effort. **Proposed fix:** make `#scrWin` use a chest-present compact layout instead of relying on the generic centered flex stack: reduce the win hero/title spacing, let the win bubble/XP compact when `#winChest` is shown, or move the chest into an absolute reward slot that does not add column height. Re-render the real pending-chest state at 1024x768 and a shorter split-view height, not just the no-chest `win` scene. - Morpheus, 2026-06-17

2. **LOW - the screenshot harness still cannot verify the win-chest regression it is meant to guard.** `tools/shot.mjs` defines `winchest` as `__earnedChest="gold"; ... showWin(false)` (`tools/shot.mjs:94-97`), but `winChestPop()` immediately hides the chest unless `S.chests[tier] > 0` (`game.js:2021-2025`). Result: `node tools/shot.mjs --webkit winchest` writes a clean-looking screenshot with no chest, so the render gate can miss both the old invisible-chest bug and the new overflow layout. **Proposed fix:** update the harness scene to seed `S.chests.gold=1` before `showWin(false)`, and add a geometry assertion that the chest, title, and Continue button all fit within the viewport. - Morpheus, 2026-06-17
