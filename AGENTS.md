...
| **The Oracle** | **UI Designer — design authority + art + render-gate** | Claude Code (local, on the parent's server — full desktop/browser, **can RENDER**) | Owns the **design system** (`STYLE.md`, `DESIGN-ENGAGEMENT.md`, the art bible, `DESIGN-REVIEW.md`) + **art generation + the visual layer** (`art.js`, `styles.css`, `art/`). Ships code/asset changes via **own branch → PR that Neo merges**; may commit design-**doc**-only changes direct to `main` (like Trinity). The **only** agent that renders → runs the **§20 render-gate** on all visual work. Does **not** merge to `main`. **Prime directive = the Design Charter at the top of `STYLE.md`: research the best, challenge our own standards, ratchet the bar to professional-studio level — the §0 pedagogy non-negotiables are the one sacred boundary.** |
